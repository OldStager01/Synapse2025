...
export default function Marquee({
  ...
  return (
    <div
      {...props}
      className={cn(
        "group relative flex h-full w-full p-2 overflow-hidden [--duration:10s] [--gap:12px] [gap:var(--gap)]",
        {
          "flex-col": vertical,
          "flex-row": !vertical,
        },
    ...
    >
      {Array.from({ length: repeat }).map((_, index) => (
        <div
          key={`item-${index}`}
          className={cn("flex shrink-0 min-w-0 [gap:var(--gap)]", {
      ...
      {applyMask && (
        <div
          className={cn(
            "pointer-events-none absolute inset-0 z-10 h-full w-full sm:from-white/50 sm:via-transparent sm:to-white/50 dark:sm:from-gray-800/50 dark:sm:via-transparent dark:sm:to-gray-800/50",
            {
